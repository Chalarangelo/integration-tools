...
const getGitMetadata = (snippet, snippetsPath) => {
  return ({
    firstSeen: execSync(`git log --diff-filter=A --pretty=format:%at -- ${snippetsPath}/${snippet} | head -1`).toString().replace('\n', ''),
    lastUpdated: execSync(`git log -n 1 --pretty=format:%at -- ${snippetsPath}/${snippet} | head -1`).toString().replace('\n', ''),
