...
const getFilesInDir = (directoryPath, withPath, exclude = null) => {
  try {
    ...
    console.log(`${red('integration-tools [ERR ]  ')} Extraction prodecure has encountered an error while getting files in a directory: ${err}`);
    process.exit(1);
  }
};
...
const readSnippets = (snippetsPath, config) => {
  ...
  try {
    ...
    console.log(`${red('integration-tools [ERR ]  ')} Extraction prodecure has encountered an error while reading snippets: ${err}`);
    process.exit(1);
  }
  return snippets;
};
