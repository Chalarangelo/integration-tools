 ...
 * TODO: Make this use the SNIPPET_PATH instead of a hardcoded `snippets/`.
 * @param snippet The name of the snippet file.
 */
const getGitMetadata = snippet => {
  return ({
    ...
    authorCount: [...new Set(execSync(`git log --pretty=%an -- snippets/${snippet}`).toString().split('\n'))].length,
  });
};

/**
 * Gets the tag array for a snippet from the tags string.
 * TODO: Deduplicate tags, but make sure the first one is still first.
 * @param tagStr The string of tags for the snippet.
 */
const getTags = tagStr => tagStr.split(',').map(t => t.trim());
