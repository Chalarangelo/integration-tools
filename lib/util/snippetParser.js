...
const getGitMetadata = (snippet, snippetsPath) => {
  return ({
    firstSeen: execSync(`git log --diff-filter=A --pretty=format:%at -- ${snippetsPath}/${snippet}`).toString(),
    lastUpdated: execSync(`git log -n 1 --pretty=format:%at -- ${snippetsPath}/${snippet}`).toString(),
